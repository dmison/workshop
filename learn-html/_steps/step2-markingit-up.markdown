So lets get started with our first page, `index.html` which is our site's homepage.
...
The very first line is the document type declaration.  This tells the browser to tell it what version of HTML to expect this document to be in.

The `html` in the declaration means "HTML version 5", which is the most current at time of writing and will probably be so for some time yet.

You might from time to time encounter declarations from older versions which are more complicated like:
...
Within the document we have two main parts, the document head and the document body.  The document head is enclosed by the `<head>` tags and can contain several other tags which provide mostly contains information about the document.  In this case we have two tags within the head `<title>` and `<meta>`

`<title>` tells the browser what the document of the document is.  Most browsers display this in the browser tab and window title and also gets used when bookmarking a page.  Notice how the browser tab has the page title now instead of the filename, this is why.
...
That sets out the basic framework of a HTML document lets go on with formatting the content in `<body>`.
